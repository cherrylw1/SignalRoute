...
const signalStages = [
  { kicker: "01 / OUTBOUND", title: "The message earns the click.", body: "A rep sends one tracked link with the account context already attached.", image: "/media/journey-context.png", alt: "Churnaut handoff view showing campaign and return context" },
  { kicker: "02 / RESOLVE", title: "The link is the identity.", body: "Churnaut recognizes the known session from the direct relationship—not an IP guess.", image: "/media/journey-visit.png", alt: "Churnaut account intelligence view showing a known buyer" },
  { kicker: "03 / PERSONALIZE", title: "The website responds.", body: "Approved headlines, copy, offers and CTAs can match the conversation that brought them there.", image: "/media/journey-action.png", alt: "Churnaut next best action view connecting signals to a teammate" },
  { kicker: "04 / ACT", title: "Scout gives the rep a reason.", body: "The return becomes evidence: person, page, timing and what changed in the deal.", image: "/media/journey-action.png", alt: "Scout next best action view for a known account" },
];
...
export function HorizontalSignalStory() {
  ...
  const phase = active === signalStages.length - 1 ? 0 : sequence - active;
  return (
    <section className="reference-horizontal" id="how-it-works" ref={ref} data-stage={active + 1} aria-labelledby="horizontal-title">
      <div className="reference-horizontal-pin">
        ...
        <div className="reference-horizontal-layout">
          <div className="reference-horizontal-copy"><p className="reference-kicker">SCROLL TO MOVE THE SIGNAL</p><div className="reference-copy-stack">{signalStages.map((stage, i) => <article key={stage.kicker} className={i === active ? "is-active" : ""}><small>{stage.kicker}</small><h2 id={i === 0 ? "horizontal-title" : undefined}>{stage.title}</h2><p>{stage.body}</p></article>)}</div></div>
          <div className="reference-horizontal-canvas"><div className="reference-horizontal-track">{signalStages.map((stage, i) => { const offset = i - active - phase; const distance = Math.abs(offset); return <figure key={stage.kicker} className={i === active ? "is-active" : ""} style={{ transform: `translate3d(0,${offset * 112}%,0) rotate(${offset * 2.2}deg) scale(${Math.max(.82, 1 - distance * .08)})`, opacity: Math.max(0, 1 - distance * .62), zIndex: 10 - Math.round(distance * 2) }}><Image src={stage.image} alt={stage.alt} fill priority={i === 0} sizes="(max-width: 900px) 92vw, 62vw" /><figcaption><span>{stage.kicker}</span><strong>{String(i + 1).padStart(2, "0")} / 04</strong></figcaption></figure>; })}</div><div className="reference-horizontal-progress"><i style={{ transform: `scaleX(${progress})` }} /></div></div>
        </div>
